...
#define	ADC_CHMUX_AGND		0x0F		/** select analog ground */
#define	ADC_CHMUX_DAC		0x10		/** select DAC's output */
#define	ADC_CHMUX_DAP		0x2F  		/** select DAP's output */

// ADC sample clock prescaler settings
#define	ADC_ADPS_DIV2		0x0			/**< Division Factor is 2 */
...
#define adcTestOn()	do { ADCSRC |= (1 << ADTM); } while(0)
#define adcTestOff()	do { ADCSRC &= ~(1 << ADTM); } while(0)
...
uint16_t adcReadChannelEx(uint8_t);
